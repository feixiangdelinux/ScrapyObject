...
class SundkiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # 把url添加到请求队列中
        for url in url_list:
            if not url.endswith('.css') and not url.endswith('.png') and not url.endswith('.xml') and not url.endswith(
                    '.m3u8') and not url.startswith(
                '#') and 'javascript' not in url and url != '/' and '</a>' not in url:
                if url.startswith('/'):
